function SPEA2N(Global)
% <algorithm> <S>
% Strength Pareto evolutionary algorithm 2 with simple normalization
...
%--------------------------------------------------------------------------
    ...
    PopObj_normalized = (PopObj - Zmin)./(Zmax - Zmin + 1e-6);
        
    Fitness    = CalFitness(PopObj_normalized);
...
%--------------------------------------------------------------------------
    ...
    if sum(Next) < N
        ...
        Next(Rank(1:N)) = true;
    elseif sum(Next) > N
        Del  = Truncation(PopObj_normalized(Next,:),sum(Next)-N);
        Temp = find(Next);
        Next(Temp(Del)) = false;
    end
